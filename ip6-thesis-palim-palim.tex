...
\begin{titlepage}
    \begin{center}
        \includegraphics[width=0.5\textwidth]{fhnw_ht_10mm}
        ...
        \vspace{0.5cm}
        \Large
        Ein Videospiel mit integriertem Videochat\\
...
\begin{abstract}	
Palim-Palim ist ein Videospiel für Kinder und betagte Menschen. Das Spiel beinhaltet einen Videochat und dient primär der Kommunikation zwischen Grosseltern und ihren Enkelkindern. Es schafft eine solide Basis für weitere Videospiele mit integriertem Videochat. Mit dem Spiel wurde der Einfluss des Videochats auf die Kommunikation und das Spielerlebnis erforscht. Spieletests mit Kindern weisen darauf hin, dass sich Videos als Gameplay-Elemente eignen. Dabei zeigte sich das grosse Potenzial von Videochat-Spielen, Beziehungen zwischen Generationen auch über weite Distanzen zu fördern.
\end{abstract}
\pagenumbering{roman}
...
\chapter{Einleitung}
Palim-Palim ist ein Videospiel mit integriertem Videochat für Kinder und betagte Menschen. Zwei Personen können damit in einer virtuellen Kaufladen-Umgebung spielen. Umgesetzt als Webanwendung für Tablets, ermöglicht das Spiel Grosseltern sich mit ihren Enkelkindern über weite Distanzen hinweg auszutauschen und gleichzeitig etwas zu spielen. Als Proof of Concept setzt Palim-Palim den Grundstein zur Entwicklung eines entsprechenden Videochat-Spiele-Frameworks. Die Abbildung \ref{PalimPalimScreens} zeigt Palim-Palim aus den Perspektiven beider Spielenden. Diese übernehmen im Spiel jeweils die Rolle der einkaufenden respektive verkaufenden Person. Das Ziel des Spiels ist es, alle Objekte einer Einkaufsliste im Warenkorb zu deponieren.
\begin{figure}[!htb]
...
Der Forschungsaspekt von Palim-Palim gewinnt dabei zusätzlich durch den Hintergrund der COVID-19-Pandemie an Relevanz. Eine amerikanische Studie zu dem Thema zeigt, dass 71 Prozent der Grosseltern von Kindern im Alter bis zu fünf Jahren aufgrund von COVID-19-Massnahmen verstärkt auf Videochats zurückgreifen \cite{kakulla_video_nodate}. Forschung von Strouse et al. \cite{strouse_zooming_2021} bestätigt ausserdem, dass Viedeochats dazu beitragen können, Grosseltern-Enkelkind-Verbindungen besser zu pflegen.\\\\
Für Eltern und Grosseltern kann es teilweise emotional anstrengend sein, wenn Kinder trotz bester Bemühungen ihrerseits im Videochat unbeteiligt erscheinen \cite{share_displaying_2017}. Denn oft sind Kinder vom Gespräch schnell gelangweilt. Für sie ist es schwierig, Gesprächsthemen und Kommunikationswege zu finden, die sich so lustig und verbindend anfühlen, wie die persönliche Zeit mit der Grossmutter oder dem Grossvater \cite{tulloch_7_2020}. Ein Videospiel wie Palim-Palim bietet hierzu ein Mittel, um Kinder besser in das Gespräch mit ihrem Gegenüber einzubeziehen.\\\\
\newpage
\noindent Um die Einflüsse eines Videochats in einem intergenerationellen Spiel zu untersuchen, implementiert Palim-Palim mehrere Varianten. Diese unterscheiden sich in der Art und Weise der Integration des Videochats. Mittels Spieletests dieser Varianten wurden deren Einflüsse auf das Spielerlebnis sowie die Kommunikation der Spielenden evaluiert und verglichen.\\\\
Die Implementierung von Palim-Palim als Webanwendung erlaubte zusätzlich die Auseinandersetzung mit den entsprechenden Videochat-Technologien. Dadurch wurde eine Wissensbasis für die Entwicklung weiterer Videochat-Spiele oder eines betreffenden Frameworks geschaffen.\\\\
Anschliessend an diese einleitenden Worte werden die theoretischen Grundlagen beschrieben. Dabei wird in Kapitel \ref{Umfeldanalyse} das Umfeld von Palim-Palim analysiert. Der Forschungsstand von intergenerationellen Spielen und Videochats in Videospielen wird in den Kapiteln \ref{IntergenerationellesSpielen} und \ref{VideochatsInVideospielen} behandelt. Dieser theoretische Teil bildet die Grundlage für die Aufstellung der Forschungsfragen in Kapitel \ref{Forschungsfragen}. Das nachfolgende Kapitel \ref{Methoden} erklärt die Spieletests. Die erzielten Resultate sind in Kapitel \ref{spieletestsUndResultate} beschrieben. Die Kapitel \ref{GameDesign}, \ref{Technologien} und \ref{Implementation} erläutern die verwendeten Technologien sowie die Umsetzung von Palim-Palim. Abschliessend werden in Kapitel \ref{Fazit} alle zentralen Erkenntnisse zusammengefasst und mögliche Weiterentwicklungen beschrieben.

% -- Theoretischer Teil --
\chapter{Umfeldanalyse}
\label{Umfeldanalyse}
Zeit zusammen zu verbringen ist mit Einflüssen wie der COVID-19-Pandemie oder auch für weit entfernt lebende Verwandte gar nicht so leicht. Immer mehr Grosseltern greifen deswegen in der Kommunikation mit ihren Enkelkindern auf Videotelefonie zurück. Dies haben Kakulla et al. \cite{kakulla_video_nodate} in einer empirischen Studie belegt. Ein steigendes Angebot sowie technische Neuerungen wie Smartphones und Tablets erlauben den betagten Personen einen deutlich leichteren Zugang zur Videotelefonie als früher \cite{glaab_silver_2015}. In einem Altersheim in Bielefeld-Senne beispielsweise wird schon seit Juli 2013 auf innovative Technologien gesetzt. Die Videotelefonie wurde dabei von den Bewohnenden sehr schnell rege genutzt, um ihre sozialen Kontakte aufrecht zu erhalten \cite{michels-ries_altenhilfe_2017}.\\\\
Wie bereits erwähnt ist Palim-Palim Teil des Forschungsprojekts Myosotis der Fachhochschule Nordwestschweiz. Die Videospiele von Myosotis sollen Familienmitgliedern, insbesondere auch Kindern, dabei helfen, mit ihren betagten Angehörigen Zeit zu verbringen \cite{soldati_fhnw_2015}. Die Spiele sollen auch dazu beitragen, durch positive soziale Interaktionen das Wohlbefinden betagter Menschen zu verbessern \cite{soldati_create_2020}. Bezüglich kameragesteuerten Spielen berichten Soldati et al. \cite{soldati_create_2020}, dass mit diesen einige der unterhaltsamsten Spieletests erlebt wurden. Allerdings wurde im Rahmen von Myosotis noch kein Spiel mit integriertem Videochat untersucht. Palim-Palim erschliesst hierbei ein neues Teilgebiet. Bisherige Forschung bezüglich intergenerationellem Spielen dient hierbei als eine Basis.\\\\

\chapter{Intergenerationelles Spielen}\label{IntergenerationellesSpielen}
Um zwischen zwei Generationen eine Brücke zu schlagen, bieten sich Videospiele als eine vielversprechende Möglichkeit an. Denn besonders im familiären Umfeld fördert gemeinsames Spielen die Beziehungen untereinander und ruft positive Emotionen sowohl bei älteren als auch jüngeren Generationen hervor \cite{osmanovic_beyond_2016}. Es hat sich gezeigt, dass Personen eine grössere Zuneigung zu Spielpartner:innen aus einer anderen Altersgruppe entwickeln und sich im Umgang mit der anderen Altersgruppe wohler fühlen, wenn sie regelmässig zusammen spielen \cite{chua_lets_2013}. Eine Literaturstudie von de la Hera et al. fasst diverse Erkenntnisse zu dem Thema übersichtlich zusammen \cite{de_la_hera_benefits_2017}. Sie identifiziert folgende vier Vorzüge des intergenerationellen Spielens: eine Stärkung der familiären Bindung, eine Förderung des wechselseitigen Lernens, ein grösseres Verständnis für die andere Generation und eine Verringerung sozialer Ängste. Unter Berücksichtigung dieser Erkenntnisse lässt sich argumentieren, dass Spielen eine wirkungsvolle Methode ist, um jüngere und ältere Menschen auf einer emotionalen, aber auch sozialen Ebene zu verbinden.\\\\
Durch den gegebenen Altersunterschied zwischen jungen und älteren Spielenden ergibt sich eine besondere Herausforderung. In der Gestaltung von intergenerationellen Spielen müssen stets zwei sehr unterschiedliche Zielgruppen berücksichtigt werden. Kognitive Fähigkeiten von betagten Personen sind tendenziell rückläufig \cite{schaie_intellectual_1996}, was in der Spielentwicklung speziell berücksichtigt werden sollte. Dazu kommen unterschiedliche Präferenzen bezüglich Spielen. Personen ab 50 Jahren bevorzugen zum Beispiel Gelegenheitsspiele gegenüber komplexeren und ausdauernden Spielen \cite{schultheiss_entertainment_2012}. Auch Gajadhar et al. \cite{gajadhar_out_2010} kamen zum Schluss, dass Spiele für Senior:innen eher weniger Fokus auf den sozialen Wettbewerb als auf das kooperative Spiel legen sollten. Diese Ergebnisse wurden von De Schutter \cite{de_schutter_meaningful_2008} bestätigt. Er stellte fest, dass soziale Interaktion der wichtigste Prädiktor für die Spieldauer bei älteren Erwachsenen ist. Betagte Menschen sind also besonders an Spielerfahrungen interessiert, die eine starke soziale Komponente haben. Dies deckt sich auch mit Empfehlungen aktueller Literatur zur Spieleentwicklung \cite{schell_art_2008}.\\\\
Kinder hingegen haben differenziertere Anforderungen an Videospiele. Es gibt in der Videospielindustrie grundlegende Empfehlungen für gewisse Altersgruppen, welche stark auf der mentalen Entwicklung beruhen. Ab dem vierten Lebensjahr beginnen Kinder ihr erstes Interesse an Spielen zu zeigen. Spiele für 4- bis 6-Jährige sind oft sehr einfach und werden häufiger mit den Eltern gespielt als mit Gleichaltrigen. Denn die Eltern wissen, wie sie die Regeln so gestalten können, dass die Spiele Spass machen und interessant sind \cite{schell_art_2008}. Ab 7 Jahren können Kinder in der Regel lesen, Dinge selbst herausfinden und auch schwierigere Probleme lösen. Sie beginnen selbst zu entscheiden, welche Art von Spielzeug und Spielen sie mögen und welche nicht \cite{schell_art_2008}. Im späteren Verlauf des Lebens kristallisieren sich dann weitere Vorlieben heraus, welche für die Zielgruppen von Palim-Palim jedoch nicht von Bedeutung sind. Es lässt sich somit ein klarer Unterschied zu den Vorlieben und Bedürfnissen älterer Leute feststellen.\\\\
Um auf die abweichenden Anforderungen von betagten Personen und Kindern einzugehen, implementiert Palim-Palim eine asymmetrische Gestaltung des Gameplays. Das heisst, beide Spielenden erhalten unterschiedliche Rollen und Aufgaben im Spiel. Die konkrete Gestaltung dieses asymmetrischen Gameplays ist im Kapitel \ref{GameDesign} ausformuliert.

\chapter{Videochats in Videospielen}
\label{VideochatsInVideospielen}
Videochats sind ein wichtiges Kommunikationsmittel um zwei oder mehr Menschen über weite Distanzen miteinander zu verbinden. Auch betagte Menschen schätzen und nutzen diese moderne Art der Kommunikation mit der Familie immer mehr – besonders weil es öfters auf ältere Benutzende angepasste Angebote gibt \cite{glaab_silver_2015}.\\\\
Während Videochats im geschäftlichen Umfeld bereits gut erforscht sind, gibt es im privaten Bereich noch Lücken, wie auch Batcheller et al. in ihrer Arbeit erwähnen \cite{batcheller_testing_2007}. In ihrer Studie haben sie Spielende des Videospiels «Mafia» über eine Videokonferenz und Spielende in einer echten Umgebung verglichen. Dabei zeigte sich, dass beide Gruppen ein ähnliches Mass an Zufriedenheit, Spass und Frustration empfanden. Dies deutet auf ein grosses Potenzial hin, Multiplayer-Spiele für die Spielenden mit Hilfe eines Videochats reichhaltiger und ansprechender zu gestalten.\\\\
...
Ausserdem hat eine Studie von Bos et al. untersucht, wie sich das Vertrauen zwischen Spielenden in einem Social Dilemma-Spiel \cite{harrod_social_1983} via Videochat entwickelt \cite{bos_being_2001}. Dabei hat sich gezeigt, dass zwischen Teilnehmenden des Videochats ein vergleichbares Vertrauen entsteht, wie in Durchgängen, welche Face-To-Face durchgeführt werden. Die Studie notiert allerdings auch, dass es verglichen mit den vor Ort durchgeführten Spielen länger dauert, bis dasselbe Niveau an Vertrauen aufgebaut wird.\\\\
Ergänzend dazu besagt eine Studie von Derboven et al. \cite{derboven_designing_2012}, dass in einem Multiplayer-Videospiel die zusätzliche Kommunikationsfunktionalität durch einen Videochat sowohl von älteren als auch von jüngeren Personen begrüsst wird. Die Videochat-Funktionalität hatte dem intergenerationellen Spiel eine zusätzliche soziale Dimension verliehen. Derboven et al. empfehlen daher in ihren abschliessenden Worten, Videochats in intergenerationellen Spielen vermehrt einzusetzen. Allerdings warnen sie auch davor, dass ein Videochat in gewissen Fällen zu unangenehmen Situationen führen kann – besonders wenn die beiden Spielenden nicht das Bedürfnis haben, miteinander zu sprechen.\\\\
Es lässt sich zusammenfassen, dass Videochats in diversen Bereichen einen wesentlichen Einfluss auf die Kommunikation zwischen zwei Parteien haben. Besonders in Videospielen, in welchen der aktive Austausch zwischen den Spielenden Teil des Spielgeschehens ist, sollte dieser Einfluss im Game Design berücksichtigt werden. Intergenerationelle Spiele können von diesen Erkenntnissen profitieren. Ein Videochat bietet eine Chance, Multiplayer-Spiele auch über Distanz ansprechend und persönlich zu gestalten.
...
Palim-Palim untersucht, wie ein Videospiel mit Videotelefonie kombiniert werden kann. Zusätzlich werden die Auswirkungen auf die Interaktionen zwischen betagten Menschen und Kindern erforscht. Daraus ergibt sich folgende übergeordnete Fragestellung:\\
\begin{quote}
...
\end{quote}
\newpage
\noindent Die verwendeten Begriffe «Kind» und «betagte Person» werden im Rahmen von Palim-Palim wie folgt definiert:
\paragraph{Kind: }Eine Person zwischen fünf und acht Jahren.
\paragraph{Betagte Person:}Eine Person ab einem Alter von 65 Jahren ohne grössere mentale Beeinträchtigung.\\\\
Diese beiden Personengruppen stellen zugleich die Zielgruppen für die Spieletests dar.

\section{Einzelfragestellungen im thematischen Zusammenhang}
Um die übergeordnete Aufgabenstellung messbar zu machen, setzt sich Palim-Palim mit spezifischen Fragestellungen zu den Themen User Experience (siehe Kapitel \ref{fragestellungenUserExperience}) und Kommunikation (siehe Kapitel \ref{fragestellungenKommunikation}) auseinander. Die dazu formulierten Hypothesen werden durch die Resultate aus den Spieltests in Kapitel \ref{spieletestsUndResultate} verifiziert oder widerlegt.

\subsection{Fragestellungen zur User Experience}\label{fragestellungenUserExperience}
Folgende Einzelfragestellungen befassen sich mit den Auswirkungen des Videochats auf das Spielerlebnis.\\\\
\textbf{1. Wie beeinflusst die Einbindung von Videotelefonie die User Experience in Videospielen?}\\\\
\textbf{Fragestellung 1a:} Welche Wirkung hat ein integrierter Videochat auf das Spielerlebnis von betagten Menschen?\\
\textbf{Hypothese 1a:} Ein integrierter Videochat hat eine positive Wirkung auf das Spielerlebnis von betagten Menschen.\\\\
\textbf{Fragestellung 1b:} Welche Wirkung hat ein integrierter Videochat auf das Spielerlebnis von Kindern?\\
\textbf{Hypothese 1b:} Ein integrierter Videochat hat eine positive Wirkung auf das Spielerlebnis von Kindern.\\\\
\newpage
\noindent 
\textbf{Fragestellung 1c:} Welche Auswirkungen hat die Art und Weise, wie der Videochat in das Spiel integriert ist, auf das Spielerlebnis?\\
\textbf{Hypothese 1c:} Je stärker der Videochat ins Gameplay integriert ist, desto positiver ist das Spielerlebnis.\\\\
...
\textbf{2. Wie beeinflusst die Einbindung von Videotelefonie die Kommunikation in Videospielen?}\\\\
\textbf{Fragestellung 2a:} Welche Wirkung hat ein integrierter Videochat auf die Kommunikation zwischen den Spielenden?\\
\textbf{Hypothese 2a:} Ein im Spiel integrierter Videochat fördert die Kommunikation zwischen den Spielenden.\\\\
\textbf{Fragestellung 2b:} Wird ein integrierter Videochat aktiv als Kommunikationsmittel zur Bewältigung von Spielaufgaben genutzt?\\
\textbf{Hypothese 2b:} Der Videochat wird aktiv als Kommunikationsmittel zur Bewältigung der Spielaufgabe verwendet.\\\\
\textbf{Fragestellung 2c:} Welche Auswirkungen hat die Art und Weise, wie der Videochat in das Spiel integriert ist, auf die Förderung der Kommunikation?\\
\textbf{Hypothese 2c:} Je stärker der Videochat ins Spiel integriert ist, desto angeregter ist der Austausch zwischen den Spielenden.\\\\
	
\chapter{Methoden}
\label{Methoden}
Wie Jesse Schell in «The Art of Game Design» \cite{schell_art_2008} beschreibt, ist die Selbstbeobachtung eine wichtige Methode, um sich schnell einen Überblick zu verschaffen, was im entwickelten Spiel funktioniert und was nicht. Dabei sollte jedoch die Gefahr der Subjektivität nicht unterschätzt werden, denn «was nach eigener Erfahrung wahr ist, mag für andere nicht wahr sein» \cite[56]{schell_art_2008}. Aus diesem Grund ist es nützlich und notwendig der Zielgruppe des Spiels sehr gut zuzuhören. Dafür eignen sich Spieletests. Dabei werden Angehörige der Zielgruppe eingeladen, das Spiel zu spielen, während sie beobachtet werden. So kann festgestellt werden, ob das beabsichtigte Spielerlebnis vermittelt wird \cite{schell_art_2008}.\\\\
Palim-Palim setzt neben der Selbstbeobachtung auf einen qualitativen statt einen quantitativen Testansatz. Dies wird einerseits damit begründet, dass die Projektzeit kein vergleichendes Testing mit mindestens 10 Personen \cite{macefield_how_2009} zuliess. Andererseits wird Palim-Palim als ein erster funktionsfähiger Prototyp und nicht als marktreifes Videospiel verstanden. Um Palim-Palim zu testen und die in Kapitel \ref{hauptfragestellung} aufgestellten Hypothesen zu prüfen, wurde ein kontrollierter Feldversuch mit zwei Testpaaren durchgeführt (betagte Menschen: 1 Frau, 1 Mann, Alter: 75-85, Kinder: 2 Jungen, 9 und 10 Jahre alt). Die teilnehmenden betagten Menschen wiesen keine grösseren gesundheitlichen Probleme auf.\\\\
...
Die Tests wurden an einem für die Teilnehmenden vertrauten Ort durchgeführt. Dadurch kann das Spiel in einer ihm zugedachten natürlichen Umgebung stattfinden, was gemäss Jesse Schell \cite{schell_art_2008} ein grosser Vorteil sein kann. Die eine Testsequenz dauerte rund 19 Minuten, die andere ca. 28 Minuten. Die Teilnehmenden erhielten vor den Spieletests eine kurze Einführung, was Palim-Palim ist und eine Erklärung, wie die Spieletests ablaufen werden (siehe Anhang \ref{InformationenZuDenSpieletests}). Ebenfalls wurden diese über die Freiwilligkeit der Tests und die Möglichkeit, die Tests jederzeit und ohne Konsequenzen abzubrechen, informiert. Alle Teilnehmenden respektive ihre Erziehungsberechtigten unterzeichneten eine Einverständniserklärung zur Verwendung ihrer Daten im Rahmen von Palim-Palim (siehe Anhang \ref{Einverständniskeitserklärung}).\\\\
Die Spieletests wurden auf Video aufgezeichnet, um die Aussagen und Handlungen der Testpersonen möglichst vollständig zu erfassen und dokumentieren. Die Teilnehmenden wurden nach jedem Videomodus gebeten, ihr Spielerlebnis zu beurteilen. Zusätzlich wurden den Testpersonen qualitative Fragen gestellt. Dabei ging es darum, Erkenntnisse zu positiven sowie negativen Erfahrungen mit den Gameplay-Elementen zu gewinnen und herauszufinden, ob Palim-Palim den Testpersonen zusagt oder nicht. Nach den drei gespielten Runden wurden die Proband:innen zusätzlich nach möglichen Ideen und Wünschen gefragt. Die Fragen wurden sehr offen gehalten, um diese auf die Teilnehmenden anpassen zu können. Der Fragebogen der Spieletests ist in Anhang \ref{Testprotokoll} zu finden.\\\\
Für die Auswertung wurden die Videoaufnahmen transkribiert und analysiert. Die Transkriptionen befinde sich in Anhang \ref{Transkription}. Zentrale Beobachtungen sowie wichtige Aussagen, Ideen und Verbesserungswünsche der Testpersonen sind im nachfolgenden Kapitel \ref{spieletestsUndResultate} dokumentiert. Ebenfalls wurden diese für mögliche Weiterentwicklungen (siehe Kapitel \ref{weiterentwicklungen}) berücksichtigt.

% -- Praktischer Teil --
\chapter{Spieletests und Resultate}\label{spieletestsUndResultate}
In diesem Kapitel werden die Ergebnisse der Spieletests von Palim-Palim präsentiert. Dabei ist sehr wichtig, diese Resultate mit besonderer Vorsicht zu geniessen. Die sehr kleine Anzahl der Testpersonen lässt es nicht zu, aussagekräftige Schlüsse aus den Resultaten zu ziehen. Es werden daher die zentralsten Aussagen und Beobachtungen zusammengefasst und mit Erkenntnisse aus der Literatur ergänzt.\\\\
Als erstes werden die Resultate aus den Spieletests mit den betagen Menschen beleuchtet. Anschliessend wird die Seite der Kinder betrachtet. Abschliessend werden die gestellten Forschungsfragen beantwortet.

\section{Resultate der Spieletests mit betagten\\ Menschen}\label{resultateBetagteMenschen}
Bereits in der gemeinsamen Gamelobby sowie auf dem Erklärungsscreen ist aufgefallen, dass die betagten Menschen eher viel Zeit für das Lesen der Anweisungen benötigen. Liu et al. \cite{liu_age-related_2017} bestätigen hierzu, dass ältere Personen einiges langsamer lesen als jüngere Menschen.\\\\
...
\newpage
\noindent Für beide Testpersonen ist unklar, dass die Abbildung mit den Erklärungen noch nicht zum Spiel gehört. Sie versuchten bereits das Einkaufsobjekt zu verschieben und die Einkaufsliste zu öffnen. Die Anweisung «Frag dein Enkelkind nach Objekten» war für eine Person unklar, denn sie fragte das Kind, was dieses gerne hätte, obwohl sie nach einem Gegenstand ihrer Einkaufsliste fragen müsste. Die Senior:innen waren sich uneinig, ob es sinnvoll ist, die Anleitung vor jedem Spielstart anzuzeigen. Eine Testperson findet dies wichtig, damit klar ist, was gemacht werden soll, die andere Person ist der Meinung, dass es mühsam sei und sie in dieser kurzen Zeit die simplen Funktionalitäten nicht vergessen habe.\\\\
...
\section{Resultate der Spieletests mit Kindern}
...
\end{quote}
Gemäss der anderen Probandin hat der integrierte Videochat ebenfalls keine grosse Auswirkung auf ihr Spielerlebnis. Jedoch findet sie den dritten Modus niedlich, da das Kind hinter dem Tresen zu stehen scheint.\\\\
Um die Fragestellung 1a eindeutig beantworten zu können, müssten eine grössere Anzahl Spieletests durchgeführt werden. Ebenfalls sollten dabei unterschiedliche Spiele berücksichtigt werden, um den Einfluss des Spieles selbst auf das Erlebnis auszuschliessen.\\\\
...
\newpage
\noindent Im Spiel selbst konnte aber auch bei dieser Fragestellung kein signifikanter Unterschied zwischen den beiden Modi mit und ohne Video festgestellt werden. Der Videochat in Palim-Palim hat die Kommunikation zwischen den Spielenden also nicht gefördert. Es muss allerdings erwähnt werden, dass Palim-Palim sehr wenige Spielelemente besitzt, welche eine Kommunikation zwischen den Spielenden erfordern. Es wäre angebracht, mehr kommunikationsfördernde Teile in das Gameplay zu integrieren, bevor hierzu eine repräsentative Aussage gemacht werden kann.\\\\
...
\caption[Caption for LOF]{Der Gameplayloop des Videospiels Palim-Palim.}
...
\begin{enumerate}
	...
	\item Spiel mit Videochat, in die Spielumgebung integriert und optionale Interaktionsmöglichkeiten mit dem Video-Stream 
	\item Spiel mit Videochat, in die Spielumgebung integriert und zum Spielerfolg notwendige Interaktionsmöglichkeiten mit dem Video-Stream
\end{enumerate}
...
\caption[Caption for LOF]{Das Videospiel Palim-Palim im Videomodus 1. Perspektive der betagten Person (links) und des Kindes (rechts).}
\label{fig:PalimPalimVideomodus1}
\end{figure}\\
\newpage
\noindent Der Screenshot des zweiten Videomodus (Abbildung \ref{fig:PalimPalimVideomodus2}) zeigt, dass in diesem Modus die Videos der Spielenden unabhängig von der Gameszene in einer Ecke des Bildschirms dargestellt werden. Identisch zum ersten Modus werden die Personen durch Illustrationen ersetzt.
\begin{figure}[!h]
\includegraphics[width=\textwidth]{PalimPalim_Videomodus_2}
\caption[Caption for LOF]{Das Videospiel Palim-Palim im Videomodus 2. Perspektive der betagten Person (links) und des Kindes (rechts).}
\label{fig:PalimPalimVideomodus2}
\end{figure}\\
Der Videomodus 3 rendert die Videos der Spielenden an die Position der ein- und verkaufenden Person. Dadurch soll der Eindruck entstehen, das Kind stünde hinter dem Tresen und verkauft Objekte. Die Grossmutter oder der Grossvater des Kindes ist analog auf der anderen Seite des Tresens platziert und spielt die Kundschaft. Dieses Szenario wird in Abbildung \ref{fig:PalimPalimVideomodus3} visualisiert.
\begin{figure}[!h]
\includegraphics[width=\textwidth]{PalimPalim_Videomodus_3}
\caption[Caption for LOF]{Das Videospiel Palim-Palim im Videomodus 3. Perspektive der betagten Person (links) und des Kindes (rechts).}
\label{fig:PalimPalimVideomodus3}
\end{figure}\\\\

\section{Design und Usability}\label{DesignUndUsability}
Wie im Kapitel \ref{spielkonzept} beschrieben, ist ein asymmetrisches Gameplay für intergenerationelle Spiele sehr geeignet. Der gesamte Prozess vom Aufruf der Website bis zum Starten des Spiels ist ebenfalls asynchron gestaltet. Die betagte Person übernimmt dabei die zentrale Rolle. Sie betritt als erstes den Videochat-Raum, startet das Spiel und wählt den Spiel- und Videomodus aus. Dies wurde aufgrund des Alters der Kinder-Zielgruppe so definiert, da sie vielleicht noch nicht lesen können. Wie im Kapitel \ref{resultateKinder} beschrieben, haben die Spieletests jedoch gezeigt, dass diese Umsetzung nicht optimal ist. Wie eine synchrone Umsetzung der Gamelobby aussehen könnte, wird als mögliche Weiterentwicklung im Kapitel \ref{weiterentwicklungen} beschrieben.\\\\
...
\chapter{Technologien}\label{Technologien}
...
Die Auswahl von WebRTC begründet sich hauptsächlich in der hohen Etablierung des Standards im Web. Zudem wird es von allen wichtigen Browsern unterstützt \cite{noauthor_can_nodate}.\\\\
Für die Umsetzung der Spiellogik wurde vorerst Unity \cite{technologies_unity_nodate} als Game-Engine in Betracht gezogen. Denn mit dem «WebRTC for Unity»-Package \cite{noauthor_webrtc_2021-1} besteht die Möglichkeit, einen Videochat in ein Unity-Spiel zu integrieren. Bei der Evaluierung der Technologien wurde klar, dass eine möglichst hohe Kontrolle über den Video-Stream benötigt wird, weil dieser interaktiv und Teil des Gameplays von Palim-Palim sein sollte. Die Ansprüche eines Spieles, in dem der Videochat eine zentrale Rolle spielt, sind Funktionen wie die Gesichtserkennung für eventuelle Filtereffekte sowie die Segmentierung des Hintergrundes zum Freistellen der Person im Video. Hierzu bietet Unity selbst wenig Unterstützung. Es braucht dafür kostenpflichtige Plugins wie das Face Tracking Plugin von Banuba \cite{noauthor_introducing_nodate}. Allerdings ist unklar, ob mit solchen Plugin-Lösungen für Unity alle Bedürfnisse von Palim-Palim erfüllt werden können. Zusätzlich wurde entschieden, keine Abhängigkeiten zu den Autor:innen dieser Plugins aufzubauen. Aufgrund dessen wurde Unity als unpassende Lösung eingestuft.\\\\
Alternativ wurde die Umsetzung von Palim-Palim als Webanwendung mit JavaScript geprüft. JavaScript ermöglicht die Nutzung von unzähligen Open-Source-Bibliotheken und bietet eine hohe Flexibilität in der Umsetzung. Bestehende Bibliotheken können beispielsweise für Gesichtsfiltereffekte \cite{noauthor_face_nodate-1} oder zur Segmentierung des Hintergrundes \cite{noauthor_bodypix_nodate} eingesetzt werden. Aus diesen Gründen wurde JavaScript zur technischen Umsetzung von Palim-Palim gewählt.\\\\
...
WebRTC ist bewusst darauf ausgelegt, auch mit komplexeren Netzwerken zurechtzukommen: Client-Anwendungen müssen NAT-Gateways (Network Address Translation) und Firewalls überwinden. Des weiteren benötigen Peer-to-Peer-Netzwerke eine Absicherung, falls die direkte Verbindung ausfällt. Als Teil dieses Prozesses verwendet die WebRTC-API zwei Netzwerkprotokolle als Hilfsmittel: 			
\begin{itemize}
  \item \textit{STUN} (Session Traversal Utilities for NAT), um die öffentliche IP-Adresse (Internet Protocol) und Portnummer für den direkten Kontaktaufbau zu ermitteln  \cite{matthews_session_2008}.
  \item \textit{TURN} (Traversal Using Relay NAT), welches die Kommunikation über NAT- oder Firewallgrenzen hinweg ermöglicht und als Fallback-Relay genutzt werden kann, falls eine Peer-to-Peer-Verbindung nicht aufgebaut werden kann \cite{matthews_traversal_2010}.
\end{itemize}
\newpage
\noindent Für beide Protokolle müssen Server bereitgestellt werden, welche den WebRTC-Clients bekannt sein müssen. Die Adressen dieser STUN- und TURN-Server müssen beim Aufbau der Peer-Connection der Clients an den Signaling-Server übermittelt werden \cite{noauthor_webrtc_nodate}. Wie dies in einer Videochat-Anwendung umgesetzt wird, ist in Abbildung \ref{webrtcNatStunFirewall} ersichtlich.\\\\
Als Fallback-Verbindung wird im produktiven Betrieb einer Anwendung wie Palim-Palim unbedingt ein eigener TURN-Server mit einer öffentlich sichtbaren IP-Adresse benötigt \cite{noauthor_webrtc_2020}. Da der Netzwerkverkehr über einen solchen Server sehr stark ansteigen kann, werden keine zuverlässigen, kostenlosen, öffentlich gehosteten TURN-Server angeboten.\\\\
Gemäss Auswertungen des WebRTC-Dienstleisters callstats.io haben im Durchschnitt etwa 30 Prozent aller WebRTC-Sessions einen Client, der sich über einen TURN-Server verbindet \cite{noauthor_why_nodate}. Das heisst in etwa 70 Prozent der Fälle kann die Peer-to-Peer-Verbindung direkt aufgebaut werden. In diesem Fall wird ein TURN-Server überhaupt nicht benötigt. Trotzdem muss bei jeder Verbindung mindestens eine TURN-Serveraddresse angegeben werden. So kann garantiert werden, dass alle Clients in der Lage sind, miteinander zu kommunizieren.\\\\
\begin{figure}[h!]
\includegraphics[width=\textwidth]{WebRTC_nat_stun_firewall_turn_black}
\caption[Caption for LOF]{STUN, TURN, und Signaling in WebRTC (Quelle: \cite{noauthor_introducing_2018} - teilweise modifiziert).}
\label{webrtcNatStunFirewall}
\end{figure}
			
\subsection{Sicherheit}
Auf der Protokollebene gilt WebRTC als sehr sicherer und auch etablierter Standard. Verschlüsselung ist für alle WebRTC-Komponenten obligatorisch und seine JavaScript-APIs können nur von sicheren Quellen wie HTTPS (Hypertext Transfer Protocol Secure) \cite{rescorla_http_2000} oder localhost aus verwendet werden \cite{noauthor_webrtc_2020-1}. Das verwendete Verschlüsselungsprotokoll hängt dabei vom Kanaltyp ab. Datenkanäle werden mit Datagram Transport Layer Security (DTLS) \cite{rescorla_datagram_2012} und Medienkanäle mit Secure Real-time Transport Protocol (SRTP) \cite{norrman_secure_2004} verschlüsselt. Die Verbindung zum Signaling-Server ist allerdings nicht standardmässig verschlüsselt. Bei Anwendungen mit Schwerpunkt auf die Vertraulichkeit sollte deshalb für die Kommunikation mit dem Signaling-Server ein sicheres Protokoll wie SIPS (Session Initiation Protocol Secure) \cite{schooler_sip_2002}, OpenSIP \cite{noauthor_opensips_nodate}, HTTPS oder WSS (WebSocket Secure) \cite{melnikov_websocket_2011} verwendet werden \cite{noauthor_study_nodate}.\\\\
Bei der Umleitung des Datenverkehrs über den TURN-Server werden die Daten nicht interpretiert oder modifiziert. Dies ist so im TURN-Standard definiert. Das heisst, die Verwendung eines TURN-Servers fügt dem WebRTC-Datenverkehr keine Sicherheitsschwachstellen hinzu.\\\\
WebRTC lässt sich somit als eigenständiges Framework für Videotelefonie als sehr sicher bezeichnen. Allerdings hängt diese Sicherheit auch von der umliegenden Webanwendung und diese wiederum vom verwendeten Browser ab. Ein im Internet zugängliches Spiel sollte deshalb über eine entsprechende Benutzerauthentifizierung verfügen, wenn es dauerhaft produktiv betrieben wird. Eine solche Authentifizierung darf nicht über den Signaling-Server laufen, denn der Datenverkehr zu ihm könnte abgefangen werden. Jede Client-Anwendung muss in der Lage sein, die Authentifizierung der potenziellen Gesprächspartner unabhängig vom Signaling-Server durchzuführen. Dies kann durch die Verwendung eines webbasierten Identitätsanbieters erreicht werden, wie zum Beispiel Facebook Connect \cite{noauthor_facebook_nodate} oder OAuth \cite{noauthor_oauth_nodate}. Ebenfalls empfiehlt es sich, die WebRTC-Bibliothek und andere Abhängigkeiten aktuell zu halten \cite{noauthor_study_nodate}.

\newpage	
\section{Three.js}\label{threejs}
Three.js ist eine 3D-Bibliothek, welche die Web Graphics Library (WebGL) \cite{noauthor_webgl_nodate-1} verwendet, um 3D-Inhalte im Browser darzustellen \cite{noauthor_threejs_nodate}. WebGL ist eine Rasterisierungs-Engine, das bedeutet, dass sie nur Punkte, Linien und Dreiecke zeichnet. Erst auf Basis des Codes entstehen dreidimensionale Objekte \cite{noauthor_webgl_nodate}. In Three.js stehen primitive 3D-Objekte genauso wie beispielsweise Lichter, Schatten, Materialien oder Texturen als vorgefertigte Ressourcen zur Verfügung \cite{noauthor_threejs_nodate}. Diese werden in einer Struktur angeordnet, welche beispielhaft in Abbildung \ref{fig:threejsstructure} ersichtlich ist.
\begin{figure}[h!]
\includegraphics[width=\textwidth]{threejs_structure}
\caption[Caption for LOF]{Die Struktur einer typischen Three.js-Anwendung \cite{noauthor_threejs_nodate}.}
\label{fig:threejsstructure}
\end{figure}\\\\
Das oberste Element einer Three.js-Struktur ist der Renderer. Dieser nimmt eine Szene (Scene) und eine Kamera (Camera) entgegen. Er zeichnet den im Blickfeld befindlichen Teil der 3D-Scene auf eine 2D-Leinwand. In der Scene werden Lichter (Light), 3D-Objekte (Object3D) und Kameras (Camera) platziert. Die Scene ist die Wurzel einer Baumstruktur, in welcher Kinder relativ zu ihren Eltern ausgerichtet werden. Mesh-Objekte sind gezeichnete Formen (Geometry) mit einem bestimmten Material. Geometry- und Material-Objekte können von verschiedenen Mesh-Objekten verwendet werden. Three.js bietet bereits einige primitive Geometry-Objekte an, wie beispielsweise Würfel, Zylinder oder Kegel. Es können aber auch eigene Formen erstellt oder aus einer Datei importiert werden. Material-Objekte repräsentieren zudem die Oberflächenbeschaffenheit eines Objekts, wie zum Beispiel die Farbe oder wie fest ein Objekt spiegelt \cite{noauthor_threejs_nodate}.

\chapter{Implementation}\label{Implementation}
In diesem Kapitel werden die einzelnen Komponenten von Palim-Palim und deren Implementierung vorgestellt. Zuerst werden in den Kapiteln \ref{PeerConnectionManager} und \ref{game} alle zentralen Client-Komponenten vorgestellt. In Kapitel \ref{server} wird anschliessend das serverseitige Backend erklärt.\\\\
Die Struktur von Palim-Palim ist bewusst so aufgebaut, dass die Videochat-Funktionalität möglichst von der Gamelogik entkoppelt ist. Dadurch ist es einfacher, den Videochat auch in anderen Spielen zu integrieren. Ebenfalls wurde damit eine möglichst solide Grundlage zur Entwicklung eines entsprechenden Frameworks gelegt. Dies spiegelt sich auch im Klassndiagramm wieder (siehe Abbildung \ref{fig:AppClassDiagram}). Die Hierarchie untersteht dabei einer zentralen App-Klasse, welche als übergeordnete Instanz das Spiel repräsentiert. Das Spiel besitzt dabei nur eine einzige Abhängigkeit zur PeerConnectionManager-Klasse, welche den Videochat implementiert.\\\\
\begin{figure}[!h]
\includegraphics[width=\textwidth]{AppClassDiagram}
\caption[Caption for LOF]{Klassendiagramm der Komponenten von Palim-Palim.}
...
Um alle diese Funktionen innerhalb der Klasse möglichst übersichtlich umzusetzen, ist die PeerConnectionManager-Klasse von Palim-Palim selbst in einzelne Unterklassen mit den entsprechenden Teilverantwortlichkeiten aufgeteilt. Diese Komponenten sind in Abbildung \ref{fig:PeerConnectionManagerDiagram} dargestellt. Die verschiedenen Zuständigkeiten der Klassen werden in den nachfolgenden Kapiteln \ref{roomManager}, \ref{videoChatManager} und \ref{dataChannelManager} vorgestellt.
\begin{figure}[!htb]
\includegraphics[width=\textwidth]{PeerConnectionManagerDiagram}
\caption[Caption for LOF]{Klassendiagramm der PeerConnectionManager-Klasse und deren Subklassen.}
\label{fig:PeerConnectionManagerDiagram}
\end{figure}\\\\
In der PeerConnectionManager-Klasse selbst findet der Aufbau der Peer-to-Peer-Verbindung statt. Die Klasse instanziiert hierzu ein PeerConnection-Objekt (blau markiert in Abbildung \ref{fig:PeerConnectionManagerDiagram}), welches die RTCPeerConnection erweitert. RTCPeerConnection ist die API, welche von WebRTC-Anwendungen verwendet wird, um eine Verbindung zwischen Peers herzustellen sowie Audio und Video zu übertragen \cite{noauthor_rtcpeerconnection_nodate-1}. Dieses PeerConnection-Objekt stellt eine WebRTC-Verbindung zwischen dem lokalen Computer und einem entfernten Client dar. Es bietet Methoden, um eine Verbindung zur Gegenseite herzustellen, die Verbindung aufrechtzuerhalten und zu überwachen sowie die Verbindung zu schliessen, wenn sie nicht mehr benötigt wird. Der PeerConnectionManager orchestriert über dieses Objekt den Verbindungsaufbau und dient als einzige Schnittstelle des Videochats zum Rest der Applikation.
...
\caption[Caption for LOF]{Nachrichtenfluss beim Betreten eines Raumes.}
\label{fig:sequenzJoinRoom}
\end{figure}\\\\
Ein typischer Nachrichtenfluss ist in Abbildung \ref{fig:sequenzJoinRoom} dargestellt. Client A beginnt den Prozess mittels einer «create or join»-Nachricht. Dies bewirkt das Erstellen eines neuen Raumes auf dem Signaling-Server. Die Nachricht enthält zusätzlich einen Raum-Identifikator. Der Server erstellt den Socket.io-Raum und bestätigt dies Client A mit einer «created»-Nachricht. Client B möchte nun den Raum mit dem gleichen Identifikator beitreten. Der Server erkennt, dass dieser Raum bereits existiert. Er schickt daher eine «joinRequest»-Nachricht an Client A. An diesem Punkt könnte von Client A die Authentifizierung des potenziellen Peers stattfinden. Palim-Palim implementiert hierzu allerdings noch keine Logik. Client B wird automatisch dem Raum von Client A zugewiesen. Über den gemeinsamen Socket.io-Raum wird anschliessend der von WebRTC vorgegebene Signaling-Prozess \cite{noauthor_webrtc_nodate-4} durchgeführt.

\subsection{VideoChatManager}\label{videoChatManager}
Nach dem Betreten eines Raumes wird über die VideoChatManager-Klasse der lokale Video- und Audiostream initiiert. Dies geschieht via Navigator \cite{noauthor_navigator_nodate}, wodurch der Browser um die Erlaubnis zur Verwendung des Mikrofons und der Kamera bittet. Erst wenn diese Erlaubnis gegeben wurde, kann auf die Daten von Kamera und Mikrofon zugegriffen werden. Der VideoChatManager fügt dann den Stream dem entsprechenden DOM-Element (Document Object Model) hinzu, wodurch die Spielenden ihr eigenes Video sehen können. Ebenfalls löst dies in Palim-Palim ein «got user media»-Event aus, wodurch der PeerConnectionManager sowie der Signaling-Server informiert werden, dass dieser Client jetzt Zugriff auf lokale Video- und Audio-Daten hat. Diese Daten werden als MediaStreamTrack-Objekt \cite{noauthor_media_nodate} der PeerConnection hinzugefügt.\\\\
...
\section{Game}
\label{game}
Neben dem PeerConnectionManager als zentrale Steuereinheit der Videochat-Funktionalität, bildet der GameManager die zweite zentrale Klasse. Er steuert die zentralen Spiel-Funktionen von Palim-Palim. Dazu gehört das Starten des Spiels, die Verwaltung der Gamelobby und die Koordination von Spielgeschehnissen. Um diese Funktionen und das komplette Handling des Spiels übersichtlich zu gestalten, besitzt der GameManager je eine Instanz von weiteren Managern. Diese übernehmen wiederum Aufgaben, welche nachfolgend genauer erläutert werden.\\\\
...
Während sich die Spielenden noch in der Gamelobby befinden, lädt Palim-Palim alle Spielmodi-übergreifenden 3D-Objekte wie den Tresen oder den Einkaufskorb. Die vom Spielmodi abhängigen Einkaufsgegenstände werden geladen, während die betagte Person die Anleitung liest und das Kind das Spiel erklärt bekommt.\\\\
...
\newpage
\noindent Diese Art der Gameplay-Synchronisation erlaubt es den Clients, total unabhängig von einem Server zu spielen. Da der Server nur fürs Signaling zuständig ist, wird eine Skalierung der Spieleranzahl vereinfacht. Der Server muss die Spielenden nur inital vermitteln. Alle Spieleinscheidungen sowie der Abgleich der Spielwelt werden von den Clients direkt vorgenommen.
		
\section{Server}\label{server}
Da Palim-Palim über das Internet zugänglich ist, wird ein Server benötigt, welcher den Client-Code zur Verfügung stellt. Der Palim-Palim-Server nutzt dafür Express.js \cite{noauthor_express_nodate}. Damit wird der JavaScript-Code den Benutzer:innen beim Aufruf der Webapplikation zur Verfügung gestellt.\\\\
...
Mit Palim-Palim wurde ein Videochat-Spiel für Kinder und betagte Menschen konzpiert und entwickelt. Anhand von Spieletests wurden Einflüsse des Spiels auf die Kommunikation und das Spielerlebnis erforscht. Dabei ist ein funktionaler Prototyp entstanden, der weiteren Videochat-Spielen als Inspiration oder als Vorlage für ein entsprechendes Framework dient. Die Spieletests haben Möglichkeiten offenbart, spielerische Elemente zu verbessern. Die mit der Arbeit verbundene Literaturrecherche zeigt ausserdem auf, dass Spiele mit integriertem Videochat ein hohes Potenzial haben, um Beziehungen zwischen Generationen auch über weite Distanzen zu pflegen.\\\\
...
Palim-Palim ist als Proof-of-Concept eines Videochat-Spiels anzusehen. Für weiterführende Forschung in diesem Feld empfielt es sich, aus den Erkenntnissen von Palim-Palim zu lernen. Eine kritische Reflexion der gewählten Ansätze und Lösungen sowie mögliche Weiterentwicklungen von Palim-Palim sind dazu in den nachfolgenden beiden Kapiteln beschrieben.

\section{Reflexion}
Testpersonen für die Spieletests von Palim-Palim wurden über einen Newsletter der Organisation «terzStiftung» \cite{noauthor_terzstiftung_nodate} gesucht. Dieser Aufruf war nicht erfolgreich. Auch sonst gestaltete sich die Suche nach Testpersonen eher schwierig. Viele angefragte Personen standen während des Testzeitraums nicht zur Verfügung. Diese Umstände führten zu einer geringen Anzahl an Spieletests. Die wenigen gefundenen Testpersonen passten zudem nicht vollständig zur Zielgruppe. Die betagten Testenden sind eher zu erfahren im Umgang mit Spielen, da sie bereits sehr viele und auch anspruchsvollere Spiele kennen. Die Kinder sind ein bis zwei Jahre zu alt. Ebenfalls sind die Testpärchen nicht verwandt miteinander. Auch sollte bei einem nächsten Spieletest Palim-Palim remote getestet werden, um die Testumgebung möglichst real zu gestalten. Bei den beiden durchgeführten Spieletests war dies aufgrund der Verfügbarkeit der Testpersonen nicht möglich.\\\\
Die Umsetzung eines Videospiels mit Vanilla JS benötigte sehr viel Zeit. Die komplette Systemarchitektur musste aufgebaut und jedes UI-Element (User Interface) von Grund auf umgesetzt werden. Rückblickend gesehen, hätte die Verwendung eines JS-Spieleframeworks, wie beispielsweise «Phaser» \cite{photonstormcom_phaser_nodate} Sinn ergeben. Damit sind geläufige Konzepte und häufig verwendete Elemente bereits implementiert und können kurzerhand verwendet werden. Weiters ist es fraglich, ob die Umsetzung in einer 3D-Welt notwendig war. Eine solche Technologie erhöht die Komplexität und damit den Zeitaufwand der Umsetzung der Spielszene.\\\\
Die Entscheidung der verwendeten Peer-to-Peer-Synchronisation sollte ebenfalls kritisch betrachtet werden. Zum Beispiel ist die Integration einer Physik-Engine so nur umständlich realisierbar. Die Physik kann nicht auf dem Server berechnet werden, da die Clients nicht mit diesem kommunizieren.
...
\subsection{Überarbeitung Raumkonzept}
Das Raumkonzept aus dem Backend wird in Palim-Palim auch im Frontend verwendet. Es wurde während der Spieletests festgestellt, dass der Austausch der Raumnummer ein Problem darstellt. Die betagte Person, welche die Raumnummer wählen kann, muss diese auf einem zweiten Kommunikationskanal dem Enkelkind übermitteln. Palim-Palim sollte daher eine Authentifizierung der Benutzenden implementieren und den Spielenden die Möglichkeit bieten, sich mit anderen registrierten Benutzenden zu verbinden. Mit einer Authentifizierung könnte zusätzlich das Problem umgangen werden, dass aus Versehen mit einer fremden Person ein Spiel gestartet wird.
\subsection{Interaktives Tutorial}
Die Spieletests haben gezeigt, dass der Erklärungsscreen durch ein interaktives Tutorial ersetzt werden sollte. Es wird vermutet, dass durch diese Art der Spielerklärung den Spielenden der Start einfacher fallen würde. Spielende können so die von ihnen verlangten Aktionen direkt ausprobieren. Ausserdem sollte das Tutorial nur einmal pro Session, und nicht bei jedem Spielstart angezeigt werden.
\subsection{Gemeinsame Menuführung}
Bezüglich der einseitigen Menuführung hat sich gezeigt, dass diese für die Kinder nicht ideal ist. In einer Weiterentwicklung von Palim-Palim sollte die Auswahl der Spielmodi gemeinsam stattfinden. Die Textbuttons sollten zusätzlich ein Bild enthalten. Beide Spielenden könnten ihren gewünschten Modus auswählen und würden ihre Wahl gegenseitig sehen. Die Auswahl würde von der betagten Person bestätigt werden.
\subsection{Verbesserung Design und Usability}
Betreffend Design und Usability gibt es ebenfalls noch einige Verbesserungsideen. Beispielsweise sollten die Verkaufsobjekte auf einem Regalboden platziert werden, um sie besser ins Spiel zu integrieren. Der Hintergrund sollte gemäss den Erkenntnissen in den Spieletests überarbeitet werden. Entweder würde sich ein interaktiver Hintergrund anbieten oder es müssten genauere Recherchearbeiten betrieben werden, wie ein Hintergrund als solcher und nicht als Teil des Gameplays erkannt wird. Das Freistellen der Person auf dem Video würde im Videomodus 3 das Video noch besser in die Spielszene integrieren. Dafür wurde bereits Recherchearbeit betrieben und eine Implementation von Zhu und Oved des Frameworks TensorFlow \cite{noauthor_bodypix_nodate} als geeignete Technologie erachtet.
\subsection{Umsetzung Videomodi 4 und 5}
Die im Konzept von Palim-Palim angedachten Videomodi 4 und 5 ermöglichen die Interaktion mit dem Video des Gegenübers. Dadurch würde im Speziellen das Spielerlebnis des Kindes attraktiver werden. Zwei Vorschläge für mögliche Szenarien sind das Aufsetzen von Verkleidungsgegenständen oder das Beschmieren mit Eis. Diese Ideen sind als Designentwurf im Anhang \ref{designEntwurfVideomodi4und5} zu finden.
\subsection{Mögliche Gameplay-Erweiterungen}
Als zusätzliche Weiterentwicklungen bieten sich einige Erweiterungen des Gameplays an. Damit soll die Motivation der Spielenden, das Spiel erneut zu spielen, gefördert werden. Einige Möglichkeiten dazu sind folgende:\\\\
\newpage
\noindent \textbf{Hinzufügen des Zahlungsprozesses}\\
Wie in Abbildung \ref{fig:PalimPalimGameplayloopBezahlen} im Anhang beschrieben, soll Palim-Palim mit dem Zahlungsprozess einen zusätzlichen Gameplayloop erhalten. Dieser würde auf den primären Gameplayloop folgen. Das heisst, nachdem sich alle benötigten Objekte im Einkaufskorb befänden, würde der Zahlungsprozess gestartet werden.\\\\
\textbf{Weitere Spielszenarien}\\
Palim-Palim bietet bereits die Möglichkeit zum Auswählen verschiedener Spielmodi. Zusätzliche Assets für die Verkaufsobjekte können via Konfigurationsdateien definiert und geladen werden. So könnte zum Beispiel ein Gemüseladen oder eine Bäckerei umgesetzt werden.\\\\
\textbf{Zusätzliche Interaktionsmöglichkeiten}\\
Es könnten zum Beispiel eine Waage für das Gemüse oder eine Tüte für die verschiedenen Brötchen implementiert werden. Damit hätte insbesondere das Kind mehrere Möglichkeiten, sich zu unterhalten.\\\\
\textbf{Unterschiedliche Spielziele}\\
Die betagte Person könnte statt konkreten Gegenständen zum Beispiel Zutaten für einen Fruchtsalat oder Festartikel für eine Geburtstagsfeier einkaufen. Dadurch kann auch dem Wunsch der Testpersonen nach Selbstbestimmung der Einkaufsgegenstände Folge geleistet werden.\\\\
\textbf{Wählbare Rollen}\\
Die Rollen sollen frei wählbar sein und die Einkaufsliste zusätzlich mit Bildern ergänzt werden. Dies würde die Kommunikation fördern, da das Kind eventuell Dinge beschreiben muss, welche es noch nicht kennt. Ebenfalls gäbe dies den Spielenden mehr Entscheidungsfreiheit.\\\\
\textbf{Physik}\\
Mit der Integration einer Physikwelt könnten Gegenstände geworfen oder über den Tresen gerollt werden. Erste Versuche mit Physik wurden bereits während der Entwicklung von Palim-Palim vorgenommen. Dabei konnte mit ammo.js \cite{zakai_ammojs_2021} und auch dem enable3D-Plugin \cite{noauthor_enable3d_nodate} für Phaser die Physik clientseitig erfolgreich implementiert werden. Allerdings scheiterten die Versuche an der Synchronisation der Objekte über die Peer-to-Peer-Verbindung. Zur Integration einer Physikwelt müsste wohl die gesamte Spiellogik auf dem Server berechnet werden. Dies würde für Palim-Palim eine grössere Umstellung bedeuten.\\\\

\subsection{Offene Probleme und Bugs}
Folgende Fehlverhalten von Palim-Palim sind bekannt und sollten bei der Weiterentwicklung des Spiels berücksichtigt werden:\\\\
\textbf{Einkaufskorb hat nur Platz für sechs Objekte}\\
Werden mehr als sechs Objekte in den Korb gelegt, werden diese nicht mehr visuell angezeigt. Die Liste wird allerdings weiter korrekt abgestrichen.\\\\
\textbf{Spiel kann nach Verlassen nicht wieder betreten werden}\\
Wird die Seite neu geladen, kann zwar der Raum erneut betrten werden und auch die Videokommunikation funktioniert noch einwandfrei, jedoch kehrt der Spielende nicht zum gestarteten Spiel zurück.\\\\
\textbf{Spiel lässt sich allein nicht abschliessen}\\
Es besteht keine Möglichkeit für die Rückkehr in die Gamelobby, wenn das Gegenüber das Spiel verlassen hat. Da der Zustand des Spiels verteilt auf beiden Clients liegt, kann das Spiel nach einem Neustart der Seite nicht wiederhergestellt werden.\\\\
\textbf{Neustart des Spiels setzt Spiel nicht zurück}\\
Das Aufräumen der Spielszene funktioniert nicht korrekt. Die Meshes verbleiben aus unbekannten Gründen in der Spielszene. Um eine zweite Runde zu spielen, muss die Seite neu geladen werden.\\\\
\textbf{Fehler beim Verschieben von Objekten}\\
Wenn beide Spielende gleichzeitig ein Objekt verschieben, führt dies teilweise zur Duplizierung des Objekts.\\\\
...
\chapter{Ideenfindung und Game Design}
\newpage
\section{Gameplayloop «Bezahlen»}
\begin{figure}[!ht]
\includegraphics[width=\textwidth]{PalimPalim_Gameplayloop_Bezahlen}
\caption[Caption for LOF]{Der angedachte Gameplayloop des Videospiels Palim-Palim für den Zahlungsprozess.}
...
\begin{figure}[!ht]
\includegraphics[width=0.8\textwidth]{Designentwurf_Videomodi45_Glace}
\caption[Caption for LOF]{Designentwurf «Glacé» für Videomodi 4 und 5. Perspektive des Kindes (links) und der betagten Person (rechts).}
\label{fig:DesignentwurfGlace}
\end{figure}

\begin{figure}[!ht]
\includegraphics[width=0.8\textwidth]{Designentwurf_Videomodi45_Verkleidung}
\caption[Caption for LOF]{Designentwurf «Verkleidung» für Videomodi 4 und 5. Perspektive des Kindes.}
\label{fig:DesignentwurfVerkleidung}
\end{figure}

\section{Weitere Spielideen}\label{WeitereIdeen}
...
\paragraph{«Drecksau»} Simples Kartenspiel mit der «Realität entsprechenden» Kartenhandlungen. Ein Beispiel für so ein Spiel ist «Drecksau» \cite{noauthor_drecksau_nodate}, bei welchem eine Regenkarte das dreckige Schweinchen wieder sauber wäscht.
...
\paragraph{Grössen-/Gewichtsverhältnisse} Die Spielenden müssen zusammen diverse Gegenstände  gemäss dem Gewicht ordnen (Baum, Elefant, Katze, Haus, Fernseher…). Das eigene Webcam-Bild kann auch einer dieser Gegenstände sein.
...
\paragraph{Labyrinth} Ein halb-transparentes Labyrinth auf dem Screen und beide Spielende starten auf verschiedenen Seiten. Mit dem Finger wird ein Pfad gezeichnet, und die Spielenden treffen müssen sich treffen.
...
\paragraph{Virtuelles Bilderbuch} Ein virtuelles Bilderbuch, das zusammen angeschaut wird. Das Kind hat dabei die Möglichkeit, mit den Szenen zu interagieren. Die ältere Person kann sich Zusatz-Informationen anzeigen lassen, und diese dem Kind vorlesen. Denkbar wären zwei Avatare, die das freigestellte Gesicht der Spielenden aus dem Videochat besitzen.
\paragraph{Wimmelbild} Ein animiertes Wimmelbild, auf dem die Spielenden sich selbst und den Gesprächspartner suchen müssen.
\paragraph{Zaubersprüche} Mittels Computer Vision werden Handbewegungen auf dem Video-Stream erkannt. In Kombination mit Audio-Erkennung könnte so ein Repertoire an Zaubersprüchen aufbaut werden. Haben die Spielenden diese gelernt, können sie lustige Filter auf den Video-Stream des Gegenüber «zaubern».

\chapter{Spieletests}\label{AnhangSpieletests}

\section{Informationen zu den Spieletests}\label{InformationenZuDenSpieletests}
\includepdf[page=-]{informationen-spieletests}

\section{Einverständniserklärung}\label{Einverständniskeitserklärung}
\includepdf[page=-]{einverstaendniserklaerung}

\section{Testprotokoll}\label{Testprotokoll}
\includepdf[page=-]{testprotokoll}

\section{Transkription Spieletests}\label{Transkription}
\includepdf[page=-]{transkription-spieletest}
...
\section{Readme}
\label{readme}
\includepdf[page=-]{README}

\section{Sequenzdiagramme}
\label{Sequenzdiagramme}
\begin{landscape}
\includepdf[page=-]{sequenzdiagram-einkaufskorb-ende}
\includepdf[page=-]{sequenzdiagram-interaktion}
\end{landscape}

\chapter{Administratives}

\section{Projektklärung}
\includepdf[page=-]{projektklaerung}

\section{Projektänderungsantrag}
\includepdf[page=-]{projektaenderungsantrag}
