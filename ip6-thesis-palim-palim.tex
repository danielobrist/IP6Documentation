...
Als weitere Weiterentwicklungen bieten sich einige Erweiterung des Gameplays an. Damit soll die Motivation der Spielenden, das Spiel erneut zu spielen, gefördert werden. Einige Möglichkeiten dazu sind:\\\\
\textbf{Hinzufügen des Zahlungsprozesses}\\
Wie in Abbildung \ref{fig:PalimPalimGameplayloopBezahlen} im Anhang beschrieben, soll Palim-Palim mit dem Zahlungsprozess einen zusätzlichen Gameplayloop erhalten. Dieser würde auf den primären Gameplayloop folgen. Das heisst, nachdem sich alle benötigten Objekte sich im Einkaufskorb befinden, würde der Zahlungsprozess gestartet.\\\\
...
Es könnten zum Beispiel ein Waage für das Gemüse oder eine Tüte für die verschiedenen Brötchen implementiert werden. Damit hätte insbesondere das Kind mehrere Möglichkeiten, sich zu unterhalten.\\\\
...
\textbf{Physik}\\
Mit der Integration einer Physik-Welt könnten Gegenstände geworfen oder über den Tresen gerollt werden. Erste Versuche mit Physik wurden bereits während der Entwicklung von Palim-Palim vorgenommen. Dabei konnte mit ammo.js \cite{zakai_ammojs_2021} und auch dem enable3D-Plugin \cite{noauthor_enable3d_nodate} für Phaser die Physik Client-seitig erfolgreich implementiert werden. Allerdings scheiterten die Versuche an der Synchronisation der Objekte über die Peer-to-Peer-Verbindung. Zur Integration einer Physikwelt müsste wohl die gesamte Spiellogik auf dem Server berechnet werden. Dies würde für Palim-Palim eine grössere Umstellung bedeuten.\\\\
...
Es besteht keine Möglichkeit für die Rückkehr in die Gamelobby, wenn das Gegenüber das Spiel verlässt. Da der Zustand des Spiels verteilt auf beiden Clients liegt, kann dieser nach einem Reload nicht wiederhergestellt werden.\\\\
\textbf{Neustart des Spiels setzt Game nicht zurück}\\
Das Aufräumen der Gamescene funktioniert nicht korrekt. Die Meshes verbleiben aus unbekannten Gründen in der Gameszene. Um eine zweite Runde zu spielen, muss die Seite neu geladen werden.\\\\
